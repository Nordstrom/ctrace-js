...
function trace(request, config) {
  return function tracedRequest(options, cb) {
    ...
    let url = options.url || options.uri && (0, _url.parse)(options.uri) || { href: 'unknown', port: 0, hostname: 'unknown' };

    let method = options.method || 'GET';

    let op = ctx.operation || config && config.getOperationName && config.getOperationName(options) || `${method || 'GET'}:${url.path}`;

    let peerService = ctx.peerService || config && config.getPeerServiceName && config.getPeerServiceName(options);
    ...
    let span = _2.default.startSpan(op, { childOf: parent, tags: tags });
